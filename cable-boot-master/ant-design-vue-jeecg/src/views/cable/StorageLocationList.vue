    ...
    methods: {
      ...
      showLocationQRCode(locationId) {
        ...
        getAction(url, { 'locationId': locationId }).then((res) => {
          if (res.success) {
            // 二维码图片预览模式  E:\web\cable\temp\code1593770642521.jpg
            // http://192.168.1.248:8080/jeecg-boot/temp\code1593770642521.jpg
            console.log('二维码图片路径>>>>>>>>', window._CONFIG['domianURL'] + '/' + res.message.substring('13'))
            window.open(window._CONFIG['domianURL'] + '/' + res.message.substring('13'))
          }
        })
      },
      modalFormOk() {

...
import org.jeecg.modules.cable.vo.StatisticalChartVo;
...
public class MaterialController extends JeecgController<Material, IMaterialService> {
    ...
    public Result<?> delete(@RequestParam(name = "id", required = true) String id) {
        ...
        wrapper.eq("raw_material_code", material.getSerial()); // 根据物料编码查询是否存在于计划表1中
        wrapper2.eq("asset_no", material.getSerial());  // 根据物料编码查询是否存在于计划表2中
        wrapper3.eq("material_code", material.getSerial()); // 根据物料编码查询是否存在于计划表3中
        ...
        // 电缆2的物料信息不可以被删除
        if (material.getSerial().equals("cable2")) {
    ...
    public Result<?> importExcel(HttpServletRequest request) {
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            try {
                List<MaterialIm> list = ExcelImportUtil.importExcel(file.getInputStream(), MaterialIm.class, params);
                List<SysDictItem> dictItems = sysDictItemService.selectType("unit");
                Material material = new Material();
                for (MaterialIm materialIm : list) {
                    // 单位转换
                    for (SysDictItem dictItem : dictItems) {
                        if (StrUtil.isNotBlank(materialIm.getUnit())) {
                            if (materialIm.getUnit().equals(dictItem.getItemText())) {
                                material.setUnit(Integer.parseInt(dictItem.getItemValue()));
                            }
                        }
                    }
                    // 属性拷贝
                    BeanUtils.copyProperties(materialIm, material);
                    materialService.save(material);
                }
